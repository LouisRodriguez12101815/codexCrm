...
export async function q<T = any>(text: string, params: unknown[] = []) { const res = await pool.query<T>(text, params); return res; }
...
CREATE TABLE IF NOT EXISTS calls (id SERIAL PRIMARY KEY,lead_id INT REFERENCES leads(id) ON DELETE SET NULL,sid TEXT,from_number TEXT,to_number TEXT NOT NULL,status TEXT NOT NULL,duration INT,recording_url TEXT,created_at TIMESTAMPTZ DEFAULT now());
